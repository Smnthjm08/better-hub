    ...
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="shrink-0 mb-4">
        ...
        <p className="text-[11px] text-muted-foreground font-mono mt-1">
          Manage your preferences, AI model configuration, and account.
        </p>
      </div>

      {/* Tab bar */}
      <div className="shrink-0 flex items-center border border-border mb-0">
        {TABS.map(({ id, label, icon: Icon }) => (
          <button
            key={id}
            onClick={() => setActiveTab(id)}
            className={cn(
              "flex items-center gap-1.5 px-4 py-2.5 text-[11px] font-mono uppercase tracking-wider transition-colors cursor-pointer",
              activeTab === id
