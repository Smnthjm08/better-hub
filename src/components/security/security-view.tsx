...
const ADVISORY_TABS: AdvisoryState[] = [
  "all",
  "triage",
  "draft",
  "published",
    ...
    advisoryTab === "all"
      ...
      : advisories.filter(
          (a) =>
            ...
            (advisoryTab === "closed" && a.state.toLowerCase() === "withdrawn")
        );
    ...
    <div className="flex gap-4 h-full min-h-0">
      ...
      <div className="flex-1 min-w-0 flex flex-col min-h-0">
        ...
        {section === "dependabot" && (
          <AlertListSection
            label="Dependabot"
            icon={<Bug className="w-3.5 h-3.5 text-muted-foreground/60" />}
            ...
            renderAlert={(alert) => (
              <>
                <p className="text-xs truncate">
                  {alert.packageName ?? "unknown"} — {alert.summary}
                </p>
        ...
        {section === "secret-scanning" && (
          <AlertListSection
            label="Secret scanning"
            icon={<KeyRound className="w-3.5 h-3.5 text-muted-foreground/60" />}
...
function AdvisoriesSection({
  ...
  const visibleTabs: AdvisoryState[] = isOwner
    ? ADVISORY_TABS
    : ["published", "closed"];
  return (
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    {advisory.severity && (
                      <span className="flex items-center gap-1.5">
                        ...
                        />
                        <span
                          className={cn("text-[11px] font-mono", sev.text)}
                        >
