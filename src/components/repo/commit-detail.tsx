...
export function CommitDetail({
  ...
  const totalAdditions = commit.stats?.additions ?? files.reduce((s, f) => s + f.additions, 0);
  const totalDeletions = commit.stats?.deletions ?? files.reduce((s, f) => s + f.deletions, 0);
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          {currentFile && (
            <FileHeader
              ...
              onNext={() => setActiveIndex(Math.min(files.length - 1, activeIndex + 1))}
            />
          )}
