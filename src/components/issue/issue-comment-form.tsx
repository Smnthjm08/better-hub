...
import { Loader2, CornerDownLeft, ChevronDown, CheckCircle2, CircleOff, CircleDot } from "lucide-react";
...
export function IssueCommentForm({ owner, repo, issueNumber, issueState, userAvatarUrl, userName, participants }: IssueCommentFormProps) {
  ...
  const handleSubmit = () => {
    ...
    setOptimisticComments((prev) => [
      ...prev,
      { id: optimisticId, body: commentBody, created_at: new Date().toISOString() },
    ]);
    setBody("");
            ...
            <div className="flex items-center gap-2">
              ...
              {isOpen ? (
                    ...
                    <div className="absolute bottom-full right-0 mb-1 w-60 bg-background border border-border rounded-md shadow-lg z-30 overflow-hidden">
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          selectedReason === "completed" && "bg-muted/40"
                        )}
                      >
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-muted-foreground/70 leading-tight">
                            Done, closed with resolution
                          </p>
                        </div>
                      </button>
                      <button
                        onClick={() => {
                          setSelectedReason("not_planned");
                          setCloseDropdownOpen(false);
                        }}
                        className={cn(
                          "flex items-center gap-2.5 w-full px-3 py-2 text-left hover:bg-muted/60 transition-colors cursor-pointer",
                          selectedReason === "not_planned" && "bg-muted/40"
                        )}
                      >
                        <CircleOff className="w-3.5 h-3.5 shrink-0 text-muted-foreground" />
                        <div>
                          <p className="text-xs font-medium">Not planned</p>
                          <p className="text-[10px] text-muted-foreground/70 leading-tight">
                            Won&apos;t fix, duplicate, or stale
                          </p>
                        </div>
                      </button>
                    </div>
                ...
                >
                  ...
                  {body.trim() ? "Reopen with comment" : "Reopen issue"}
                </button>
              )}
